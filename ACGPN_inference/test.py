...
with torch.no_grad():
    for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            losses, fake_image, real_image, input_label, L1_loss, style_loss, clothes_mask, CE_loss, rgb, alpha = model(Variable(data['label']), Variable(data['edge']), Variable(img_fore), Variable(mask_clothes),
                                                            Variable(data['color']), Variable(all_clothes_label), Variable(data['image']), Variable(data['pose']), Variable(data['image']), Variable(mask_fore))
            ...
            if step % 1 == 0:
                ...
                n = str(step)+'.jpg'
                print(data['name'][0])
                cv2.imwrite(os.path.join(r'sample', data['name'][0]), bgr)
            step += 1
            print(step)
            # ## save latest model
